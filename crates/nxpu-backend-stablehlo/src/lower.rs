...
fn build_attention_mlir(
    ...
    let scale: f32 = 1.0 / (d_k.parse::<f32>().unwrap_or(64.0)).sqrt();
    let scale_str = format!("{scale:.8e}");

    format!(
        r#"// StableHLO module generated by nxpu
// Entry point: {ep_name}
// Pattern: Attention (scaled dot-product)
module @{ep_name} {{
  func.func @main(%{q}: {mat_type}, %{k}: {mat_type}, %{v}: {mat_type}) -> {mat_type} {{
    ...
    // scale by 1/sqrt(d_k)
    %scale = stablehlo.constant dense<{scale_str}> : {scalar_type}
    %scale_bcast = stablehlo.broadcast_in_dim %scale, dims = [] : ({scalar_type}) -> {mat_type}
    ...
    %max_scores = stablehlo.reduce(%scores_scaled init: %neg_inf) applies stablehlo.maximum across dimensions = [1] : ({mat_type}, {scalar_type}) -> {vec_type}
    %max_bcast = stablehlo.broadcast_in_dim %max_scores, dims = [0] : ({vec_type}) -> {mat_type}
    %shifted = stablehlo.subtract %scores_scaled, %max_bcast : {mat_type}
    ...
    %{out} = stablehlo.dot_general %attn_weights, %{v},
      batching_dims = [] x [],
      contracting_dims = [1] x [0]
      : ({mat_type}, {mat_type}) -> {mat_type}
    return %{out} : {mat_type}
