...
fn build_split_mlir(
    ...
    for (i, o) in outputs.iter().enumerate() {
        slices.push(format!(
            "    %{} = stablehlo.slice %{} [{}:1:?] : ({}) -> {}",
            o.name, input.name, i, tensor_type, tensor_type
        ));
    }
...
fn build_attention_mlir(
    ...
    let score_type = format!("tensor<?x?x{ty}>");
...
module @{ep_name} {{
  func.func @main(%{q}: {mat_type}, %{k}: {mat_type}, %{v}: {mat_type}) -> {mat_type} {{
    %scores = stablehlo.dot_general %{q}, %{k},
      ...
      : ({mat_type}, {mat_type}) -> {score_type}
    %init = stablehlo.constant dense<0.0> : tensor<{ty}>
    %sum = stablehlo.reduce(%scores init: %init) applies stablehlo.add across dimensions = [1] : ({score_type}, tensor<{ty}>) -> tensor<?x{ty}>
    %{out} = stablehlo.dot_general %scores, %{v},
      batching_dims = [] x [],
      contracting_dims = [1] x [0]
      : ({score_type}, {mat_type}) -> {mat_type}
    return %{out} : {mat_type}
  }}
}}
"#,
