...
module @{ep_name} {{
  func.func @main(%{q}: {mat_type}, %{k}: {mat_type}, %{v}: {mat_type}) -> {mat_type} {{
    ...
    // scale by 1/sqrt(d_k) — use a placeholder constant
    %scale = stablehlo.constant dense<1.0> : {scalar_type}
    %scores_scaled = stablehlo.multiply %scores, stablehlo.broadcast_in_dim(%scale, dims = []) : ({mat_type}, {mat_type}) -> {mat_type}
